...
/*
	...
	*/
		...
		if(empty($thevalue) || $thevalue['uid'] != $_G['uid']) {
			BfdApp::display_result('favorite_does_not_exist');
		}
...
//	}
	...
	if(empty($idtype) || empty($title)) {
		BfdApp::display_result('favorite_cannot_favorite');
	}

	$fav = C::t('home_favorite')->fetch_by_id_idtype($id, $idtype, $_G['uid']);
	if($fav) {
		BfdApp::display_result('favorite_repeat');
	}
	$description = $extrajs = '';
...
//	if(submitcheck('favoritesubmit') || $type == 'forum' || $type == 'group') {
		...
		BfdApp::display_result('favorite_do_success',array('favid'=>$favid));
//	}
}
