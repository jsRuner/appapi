...
//	$announcements = get_index_announcements();
	...
	foreach($forums as $forum) {
		...
		if(!empty($forum['icon']))
		{
			...
			$forum['icon'] = BFD_APP_DATA_URL_PRE.getglobal('setting/attachurl').'common/'.$forum['icon'];
		}
		else
		{
			$forum['icon'] = BFD_APP_DATA_URL_PRE.'static/image/common/forum.gif';
		}
/*		if(empty($forum['description']))
		{
			$forum['description'] = '本板块还没有说明哦';
		}
*/
		//unset forum fields whr
		unset($forum['styleid']);
		unset($forum['allowsmilies']);
		unset($forum['allowhtml']);
		unset($forum['allowbbcode']);
		unset($forum['allowimgcode']);
		unset($forum['allowmediacode']);
		unset($forum['allowanonymous']);
		unset($forum['allowpostspecial']);
		unset($forum['allowspecialonly']);
		unset($forum['allowappend']);
		unset($forum['alloweditrules']);
		unset($forum['allowfeed']);
		unset($forum['allowside']);
		unset($forum['recyclebin']);
		unset($forum['modnewposts']);
		unset($forum['jammer']);
		unset($forum['disablewatermark']);
		unset($forum['inheritedmod']);
		unset($forum['autoclose']);
		unset($forum['forumcolumns']);
		unset($forum['catforumcolumns']);
		unset($forum['threadcaches']);
		unset($forum['alloweditpost']);
		unset($forum['simple']);
		unset($forum['modworks']);
		unset($forum['allowglobalstick']);
		unset($forum['commoncredits']);
		unset($forum['archive']);
		unset($forum['recommend']);
		unset($forum['favtimes']);
		unset($forum['sharetimes']);
		unset($forum['disablethumb']);
		unset($forum['disablecollect']);
		unset($forum['attachextensions']);
		unset($forum['creditspolicy']);
		unset($forum['formulaperm']);
		unset($forum['moderators']);
		unset($forum['rules']);
		unset($forum['threadtypes']);
		unset($forum['threadsorts']);
		unset($forum['viewperm']);
		unset($forum['postperm']);
		unset($forum['replyperm']);
		unset($forum['getattachperm']);
		unset($forum['postattachperm']);
		unset($forum['postimageperm']);
		unset($forum['spviewperm']);
		unset($forum['seotitle']);
		unset($forum['keywords']);
		unset($forum['seodescription']);
		unset($forum['supe_pushsetting']);
		unset($forum['modrecommend']);
		unset($forum['threadplugin']);
		unset($forum['replybg']);
		unset($forum['extra']);
		unset($forum['gviewperm']);
		unset($forum['commentitem']);
		unset($forum['relatedgroup']);
		unset($forum['picstyle']);
		unset($forum['widthauto']);
		unset($forum['noantitheft']);
		unset($forum['noforumhidewater']);
		unset($forum['noforumrecommend']);
		unset($forum['livetid']);
		unset($forum['price']);
		unset($forum['permission']);
		unset($forum['lastpost']);
		unset($forum['banner']);
		unset($forum['groupnum']);
		unset($forum['permission']);
		unset($forum['folder']);
		unset($forum['moderators']);
		unset($forum['redirect']);
		unset($forum['lastupdate']);
		unset($forum['rank']);
		unset($forum['oldrank']);
		unset($forum['domain']);
		unset($forum['jointype']);
		$forum['password'] = empty($forum['password']) ? 0 : 1;
		$forum['name'] = strip_tags($forum['name']);
		$forum['name'] = BfdApp::bfd_html_entity_decode($forum['name']);
		//unset end
		if($forum['type'] != 'group') {
			...
			//	}
				...
				$forumlist[$forum['fup']]['subforums'][] = $forum;
...
//				}
	...
	{
		$catlist[$value['fup']]['subforums'][] = $value;
	}

//	var_dump($catlist);
//	exit;
//$result = array_values($forumlist);
$result = array_values($catlist);
$pagetotal = array();
loadcache('userstats');
//$pagetotal['postinfo'] = "今日:{$todayposts}  |  昨日:{$postdata[0]}  |  帖子:{$posts}  |  会员:{$_G['cache']['userstats']['totalmembers']}";
$pagetotal['postinfo'] = "今日:{$todayposts}  |  昨日:{$postdata[0]}  |  会员:{$_G['cache']['userstats']['totalmembers']}";
//今日: 1|昨日: 8|帖子: 300|会员: 89
...
if( BFD_APP_CHARSET_OUTPUT != BFD_APP_CHARSET ){
    $pagetotal['postinfo'] = iconv( BFD_APP_CHARSET_OUTPUT, BFD_APP_CHARSET."//IGNORE", $pagetotal['postinfo'] );
}
...
//BfdApp::display_result('get_success',$forumlist);
