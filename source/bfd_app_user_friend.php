...
//		BfdApp::display_result('no_privilege_addfriend');
//	}

	if($uid == $_G['uid']) {
		BfdApp::display_result('friend_self_error');
	}

	if(friend_check($uid)) {
		BfdApp::display_result('you_have_friends');
	}
	...
	if($maxfriendnum && $space['friends'] >= $maxfriendnum + $space['addfriend']) {
		if($_G['magic']['friendnum']) {
			BfdApp::display_result('enough_of_the_number_of_friends_with_magic');
		} else {
			BfdApp::display_result('enough_of_the_number_of_friends');
		}
	}
	...
	if(friend_request_check($uid)) {
			...
			BfdApp::display_result('add_friend_success');
			//成功输出

	} else {

		if(C::t('home_friend_request')->count_by_uid_fuid($uid, $_G['uid'])) {
			BfdApp::display_result('waiting_for_the_other_test');
		}
		...
		*/
			...
			BfdApp::display_result('request_has_been_sent');
	...
	//	if(submitcheck('friendsubmit')) {
			...
			BfdApp::display_result('ignore_friend_success');
	//	}
	...
	BfdApp::display_result('ignore_friend_failed');
...
*/
	...
	if($space['friends']) {
		...
		$count = C::t('home_friend')->fetch_all_search($uid, $group, '', true);
		if($count) {
			$query = C::t('home_friend')->fetch_all_search($uid, $group, '', false, $start, $perpage, true);
			foreach($query as $value) {
